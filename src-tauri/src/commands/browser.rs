...
#[tauri::command]
pub async fn inbox_open(app: tauri::AppHandle) -> Result<String, String> {
    let base = app.path().app_data_dir().map_err(|e| e.to_string())?;
    let profile = base.join("inbox-profile");
    fs::create_dir_all(&profile).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn inbox_wipe(app: tauri::AppHandle) -> Result<(), String> {
    let base = app.path().app_data_dir().map_err(|e| e.to_string())?;
    let profile = base.join("inbox-profile");
    if profile.exists() {
...
pub fn resolve_chromium_public(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    resolve_chromium(app)
}

pub fn find_privacy_badger_public(app: &tauri::AppHandle) -> Option<PathBuf> {
    find_privacy_badger(app)
}
...
fn resolve_chromium(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(dir) = app.path().resource_dir() {
        ...
        candidates.push(dir.join("resources").join("chromium").join(plat).join(exe_name));
        candidates.push(dir.join("chromium").join(plat).join(exe_name));
    }
    if let Ok(exe) = std::env::current_exe() {
        let exe = dunce::simplified(&exe).to_path_buf();
        if let Some(parent) = exe.parent() {
            candidates.push(parent.join("resources").join("chromium").join(plat).join(exe_name));
            candidates.push(parent.join("chromium").join(plat).join(exe_name));
        }
    }
    ...
    Err(format!(
        "chromium not found. tried: {:?}. Drop it into <app>/resources/chromium/{}/ or set BOTCORD_CHROMIUM.",
        candidates, plat
    ))
}
