...
export function FriendModal({
  ...
  useEffect(() => {
    ...
    listen<SocialStep>('friend:progress', (e) => {
  ...
  const fire = async () => {
    if (running || !raw.trim() || accountIds.length === 0) return
    setRunning(true)
    setSteps([])
    setResult(null)
    ...
    try {
      const r = await api.bulkFriendRequest({
        ...
        >
          ...
          >
            <div className="h-10 px-4 flex items-center gap-3 border-b border-line">
              <span className="text-[11px] uppercase tracking-[0.22em]">friend request</span>
            ...
            <div className="p-4 flex flex-col gap-3">
              <label className="flex flex-col gap-1">
                ...
                <input
                  ...
                  placeholder="username or legacy name#0001"
              ...
              <div className="flex items-center justify-between pt-1">
                <div className="text-[10px] uppercase tracking-[0.22em] text-ink-3">
                  {running
                    ...
                    : result
                    ? `sent ${result.ok} / fail ${result.failed}`
                    : 'ready'}
                </div>
                <button
                  onClick={fire}
                  disabled={running || accountIds.length === 0 || !raw.trim()}
              ...
              <div className="label">
                new usernames (no discriminator) just paste the handle. legacy name#0001 also works.
              </div>
                  ...
                  {steps.map((r, i) => (
                    ...
                    >
                      <span
                        className={clsx(
                          'w-1.5 h-1.5',
                          r.state === 'sent' ? 'bg-ok' : r.state === 'error' ? 'bg-bad' : 'bg-warn animate-pulse',
                        )}
                      />
