...
export function Servers() {
  ...
  const clearAll = useServers((s) => s.clearAll)
        ...
        {channels.length > 0 && (
          <div className="flex items-center gap-2">
            <span className="text-[10px] uppercase tracking-[0.22em] text-ink-3">
              {channels.length} channels · {guilds.length} guilds
            </span>
            <button
              onClick={async () => {
                if (!confirmClear) {
              ...
              className={clsx(
                'ml-auto h-7 px-3 text-[10px] uppercase tracking-[0.22em] border transition',
                confirmClear
                  ? 'border-bad text-bad bg-bad/10'
                  : 'border-line-2 text-ink-3 hover:text-bad hover:border-bad',
              )}
            >
